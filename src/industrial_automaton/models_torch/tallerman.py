...
class Tallerman(BaseAutomata):
    ...
    def __init__(self, config: TallermanConfig, generator: torch.Generator = None):
        ...
        # Content-based attention read — per-head queries, initialized identically for stability
        W_q_base = torch.randn(config.memory_cell_size, config.hidden_size, generator=generator) * scale
        self.W_q = nn.Parameter(W_q_base.unsqueeze(0).expand(config.num_heads, -1, -1).clone())
    ...
    def _step(self, x_t: torch.Tensor, state: TallermanState, jump_len: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Content-based attention read — per-head queries
